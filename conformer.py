...
class LDELayer(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        self.conv_c=nn.Sequential(nn.Conv2d(384,576,1,1),self.relu)
        self.conv_c1=nn.Sequential(nn.Conv2d(768,576,1,1),self.relu)
        self.conv_c2=nn.Sequential(nn.Conv2d(1536,576,1,1),self.relu)
        self.conv_rgb=nn.Sequential(nn.MaxPool2d(3),nn.Conv2d(384, 64, 7, 1, 1), nn.Conv2d(64, 64, 5, 1, 1), self.relu)
        self.conv_rgb1=nn.Sequential(nn.MaxPool2d(2),nn.Conv2d(768, 64, 5, 1, 1), nn.Conv2d(64, 64,3, 1, 2), self.relu)
        self.conv_rgb2=nn.Sequential(nn.MaxPool2d(1),nn.Conv2d(1536, 64, 3, 1, 1), nn.Conv2d(64, 64,1, 1, 0), self.relu)
    ...
    def forward(self, list_x,list_y,q,k,v):
        ...
        edge1=depth_lde[1]+rgb_lde[1]
        edge2=depth_lde[2]+rgb_lde[2]'''
        '''j=8
...
class Decoder(nn.Module):
    ...
    def forward(self, rgb_e,depth_e ,rgb_h,rgb_m,depth_h,depth_m,rgb_l,depth_l):
        ...
        edge_rgbd2=self.act(self.up2(self.up21(torch.cat(((rgb_lde2+depth_lde2),(rgb_lde2*depth_lde2)),dim=1))))
...
class JL_DCF(nn.Module):
    ...
    def forward(self, f_all,f1_all):
        ...
        rgb_e,depth_e = self.lde(x,y,q,k,v)
        ...
        sal_final,sal_low,sal_med,sal_high,e_rgbd0,e_rgbd1,e_rgbd2=self.decoder(rgb_e,depth_e ,rgb_h,rgb_m,depth_h,depth_m,rgb_l,depth_l)
